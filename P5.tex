...
\section{Data Analysis}
    ...
    For Hypothesis 1, the goal is to determine whether participants can efficiently complete the core match-finding workflow on Pickle Cloud. Task completion will be recorded as a binary outcome (\emph{completed} or \emph{not completed}). Frequency counts and percentages of completions will be calculated. A high completion rate (≥90\%) will support the hypothesis that Pickle Cloud improves task efficiency.

    For Hypothesis 2, the goal is to assess interface clarity and user satisfaction. Each of the three core tasks (view/select court, find match, start chat) will be recorded as completed or not. Participants will also provide satisfaction ratings on a 0–100\% scale. Frequency counts of task completions and the mean satisfaction score will be reported. High completion rates and an average satisfaction score ≥85\% will support the hypothesis that the interface is intuitive and satisfying to use.

    For Hypothesis 3, the goal is to identify where participants may encounter difficulties in completing the workflow. Each task will be broken into sub-goals, and completion will be averaged across these sub-goals for each participant. The mean and standard deviation of these averages will be calculated. A higher average completion rate with the prototype will support the hypothesis that the interface effectively guides users through all steps of the workflow.
